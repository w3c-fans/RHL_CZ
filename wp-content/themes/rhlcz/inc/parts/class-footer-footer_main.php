...
if ( ! class_exists( 'TC_footer_main' ) ) :
    ...
    */
    function tc_footer_hook_setup() {
      ...
      //footer actions
      add_action ( '__footer'         , array( $this , 'tc_colophon_display' ), 10 );
      add_action ( '__footer'         , array( $this , 'tc_widgets_footer' ), 20 );
	    	...
	    	echo apply_filters(
	    		...
	    		sprintf('<div class="%1$s">%2$s</div>',
		    		...
		    		sprintf( '<p>%1$s %2$s %3$s</p>',
						    apply_filters( 'tc_copyright_link', sprintf( '<span class="tc-copyright-text">&copy; %1$s</span> <a href="%2$s" title="%3$s" rel="bookmark">%3$s</a>', esc_attr( date( 'Y' ) ), esc_url( home_url() ), esc_attr( get_bloginfo() ) ) ),
                            apply_filters( 'tc_credit_link', sprintf( '' ) ),
